...
(() => {
  ...
  var init_convertLrgbToOklab = __esm({
    "node_modules/culori/src/oklab/convertLrgbToOklab.js"() {
      convertLrgbToOklab = ({ r: r2, g, b, alpha }) => {
        if (r2 === void 0)
          r2 = 0;
        if (g === void 0)
          g = 0;
  ...
  var init_convertRgbToYiq = __esm({
    "node_modules/culori/src/yiq/convertRgbToYiq.js"() {
      convertRgbToYiq = ({ r: r2, g, b, alpha }) => {
        if (r2 === void 0)
          r2 = 0;
        if (g === void 0)
          g = 0;
  ...
  var r, round, round_default;
  var init_round = __esm({
    "node_modules/culori/src/round.js"() {
      r = (value, precision) => Math.round(value * (precision = Math.pow(10, precision))) / precision;
      round = (precision = 4) => (value) => typeof value === "number" ? r(value, precision) : value;
      round_default = round;
    }
  });

  // node_modules/culori/src/formatter.js
  var twoDecimals, clamp, fixup, rgb, hsl, serializeHex, formatHex;
  var init_formatter = __esm({
    "node_modules/culori/src/formatter.js"() {
      init_converter();
      init_round();
      twoDecimals = round_default(2);
      clamp = (value) => Math.max(0, Math.min(1, value || 0));
      fixup = (value) => Math.round(clamp(value) * 255);
      ...
      formatHex = (c2) => serializeHex(rgb(c2));
    }
  });

  // node_modules/culori/src/index.js
  var a98, cubehelix, dlab, dlch, hsi, hsl2, hsv, hwb, itp, jab, jch, lab, lab65, lch, lch65, lchuv, lrgb, luv, okhsl, okhsv, oklab, oklch, p3, prophoto, rec2020, rgb2, xyb, xyz50, xyz65, yiq;
  var init_src = __esm({
    "node_modules/culori/src/index.js"() {
      init_definition2();
      init_definition3();
      ...
      hsl2 = useMode(definition_default7);
  ...
  // lib/color-utils.ts
  function hexToOklch(hex2) {
    var _a, _b, _c;
    const color = oklch(hex2);
    if (!color) {
      return { l: 0.5, c: 0.1, h: 0 };
    }
    return {
      l: (_a = color.l) != null ? _a : 0,
      c: (_b = color.c) != null ? _b : 0,
      h: (_c = color.h) != null ? _c : 0
    };
  }
  ...
  function getRelativeLuminance(hex2) {
    var _a, _b, _c;
    const color = rgb2(hex2);
    if (!color)
      return 0;
    const toLinear = (c2) => c2 <= 0.03928 ? c2 / 12.92 : Math.pow((c2 + 0.055) / 1.055, 2.4);
    const r2 = toLinear((_a = color.r) != null ? _a : 0);
    const g = toLinear((_b = color.g) != null ? _b : 0);
    const b = toLinear((_c = color.b) != null ? _c : 0);
  ...
  function findLightnessForContrast(baseColor, background, targetContrast) {
  ...
  function generateColor(baseColor, stop, stopData, globalMode, globalLightness, globalContrast, backgroundColor, perceptualCorrections, hueShiftAmount, hueShiftDirection, chromaShiftAmount, chromaShiftDirection) {
    ...
    if (effectiveMode === "contrast" && backgroundColor) {
      const targetContrast = (_b = (_a = stopData == null ? void 0 : stopData.contrast) != null ? _a : globalContrast == null ? void 0 : globalContrast[stop]) != null ? _b : 4.5;
      targetL = findLightnessForContrast(baseOklch, backgroundColor, targetContrast);
    } else {
      if ((stopData == null ? void 0 : stopData.lightness) !== void 0) {
        targetL = stopData.lightness;
      } else if (globalLightness && globalLightness[stop] !== void 0) {
    ...
    if (hueShiftAmount && hueShiftAmount > 0) {
      const effectiveShift = hueShiftDirection === "cool-warm" ? -hueShiftAmount : hueShiftAmount;
  ...
  function hexToRgb(hex2) {
    ...
    const color = rgb2(hex2);
    return {
      r: Math.round(((_a = color == null ? void 0 : color.r) != null ? _a : 0) * 255),
      g: Math.round(((_b = color == null ? void 0 : color.g) != null ? _b : 0) * 255),
      b: Math.round(((_c = color == null ? void 0 : color.b) != null ? _c : 0) * 255)
    };
  }
  function getHKCompensation(chroma, hue3) {
    const hueFactor = 0.5 + 0.5 * Math.cos((hue3 - 270) * Math.PI / 180);
    const chromaFactor = Math.min(chroma / 0.2, 1);
    return 0.05 * chromaFactor * hueFactor;
  }
  function applyHKCompensation(color, isLightBackground) {
    const compensation = getHKCompensation(color.c, color.h);
    const adjustedL = isLightBackground ? color.l - compensation : color.l + compensation;
    return __spreadProps(__spreadValues({}, color), { l: Math.max(0, Math.min(1, adjustedL)) });
  }
  function getBBShiftCorrection(baseHue, lightness) {
    const midPoint = 0.5;
  ...
  function applyBBCorrection(color) {
    const hueCorrection = getBBShiftCorrection(color.h, color.l);
    const correctedHue = (color.h + hueCorrection + 360) % 360;
    return __spreadProps(__spreadValues({}, color), { h: correctedHue });
  }
  function applyHueShift(color, targetL, shiftAmount) {
    if (shiftAmount === 0)
      return color;
    const normalizedL = (targetL - 0.5) * 2;
    const hueOffset = -normalizedL * (shiftAmount / 2);
    const newHue = (color.h + hueOffset + 360) % 360;
    return __spreadProps(__spreadValues({}, color), { h: newHue });
  }
  function applyChromaShift(color, targetL, shiftAmount, direction = "vivid-muted") {
    if (shiftAmount === 0 || color.c === 0)
      return color;
    const normalizedL = (targetL - 0.5) * 2;
    let reductionFactor;
    ...
    return __spreadProps(__spreadValues({}, color), { c: newChroma });
  }
  function applyPerceptualCorrections(color, backgroundColor, options = {}) {
    let result = __spreadValues({}, color);
  ...
  // lib/figma-utils.ts
  function hexToFigmaRgba(hex2) {
    const rgb3 = hexToRgb(hex2);
    return {
      r: rgb3.r / 255,
      g: rgb3.g / 255,
      b: rgb3.b / 255,
  ...
  function getOrCreateCollection(name) {
    return __async(this, null, function* () {
      ...
      const existing = collections.find((c2) => c2.name === name);
      if (existing) {
        return existing;
      }
  ...
  function getOrCreateColorVariable(collection, name) {
    return __async(this, null, function* () {
      const existingVariables = yield figma.variables.getLocalVariablesAsync("COLOR");
  ...
  function createFigmaVariables(colors, globalSettings) {
    return __async(this, null, function* () {
      var _a, _b;
      const collection = yield getOrCreateCollection("Octarine Colors");
      ...
      for (const color of colors) {
        for (const stop of color.stops) {
          ...
          const hexColor = generateColor(
            ...
            String(stop.number),
            {
              lightness: (_a = stop.lightnessOverride) != null ? _a : lightness[stop.number],
              contrast: (_b = stop.contrastOverride) != null ? _b : contrast[stop.number],
          ...
          const variable = yield getOrCreateColorVariable(collection, variableName);
          const isNew = variable.name === variableName && Object.keys(variable.valuesByMode).length === 0;
        ...
        switch (msg.type) {
          ...
          case "create-variables":
            try {
              ...
              const globalSettings = msg.globalSettings;
              const result = yield createFigmaVariables(colors, globalSettings);
              const message = result.created > 0 ? `Created ${result.created} variables, updated ${result.updated}` : `Updated ${result.updated} variables`;
          ...
          case "get-selection-color":
            ...
            if ("fills" in node && Array.isArray(node.fills) && node.fills.length > 0) {
              ...
              if (fill.type === "SOLID") {
                const r2 = Math.round(fill.color.r * 255);
                const g = Math.round(fill.color.g * 255);
                const b = Math.round(fill.color.b * 255);
                const hex2 = "#" + [r2, g, b].map((x) => x.toString(16).padStart(2, "0")).join("");
                figma.ui.postMessage({ type: "selection-color", color: hex2 });
              } else {

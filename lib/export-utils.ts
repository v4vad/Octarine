...
export function generateTailwindConfig(colors: Color[], globalConfig: GlobalConfig): string {
  ...
  for (const stop of stops) {
    const key = stop.colorLabel.toLowerCase().replace(/\s+/g, '-')
...
export function generateSCSS(colors: Color[], globalConfig: GlobalConfig): string {
  ...
  for (const stop of stops) {
    const label = stop.colorLabel.toLowerCase().replace(/\s+/g, '-')
    lines.push(`$${label}-${stop.stopNumber}: ${stop.hex.toUpperCase()};`)
  }
  return lines.join('\n')
}
